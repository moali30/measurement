...
export default function FormsListPage() {
  ...
  const filtered = forms.filter(f => f.title.toLowerCase().includes(search.toLowerCase()) && (statusFilter==="all"||f.status===statusFilter));
  ...
  const badge = (s:string) => {
    ...
    if(s==="draft") return <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-100"><Edit3 size={12}/>مسودة</span>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[{l:"إجمالي",v:stats.total,c:"bg-blue-50 text-blue-500"},{l:"نشط",v:stats.active,c:"bg-emerald-50 text-emerald-500"},{l:"مسودة",v:stats.draft,c:"bg-amber-50 text-amber-500"},{l:"الردود",v:stats.responses,c:"bg-purple-50 text-purple-500"}].map((s,i)=>(
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex border border-gray-200 rounded-xl overflow-hidden bg-white">
          {[{k:"all",l:"الكل"},{k:"active",l:"نشط"},{k:"draft",l:"مسودة"}].map(f=>(
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(form=>(
            <div key={form.$id} className="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-gray-200 transition-all">
              <div className={`h-1.5 rounded-t-2xl ${form.status==='active'?'bg-gradient-to-l from-emerald-400 to-emerald-500':'bg-gradient-to-l from-amber-300 to-amber-400'}`}/>
              <div className="p-5">
                  ...
                  <div className="relative">
                    ...
                    {openMenuId===form.$id&&(
                      <div className="absolute left-0 top-full mt-1 w-48 bg-white rounded-xl shadow-2xl border border-gray-200 py-1.5 z-50">
                        ...
                        <button onClick={()=>toggleStatus(form)} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm hover:bg-gray-50 text-gray-700">{form.status==='active'?<><EyeOff size={14} className="text-amber-500"/>إيقاف</>:<><Eye size={14} className="text-emerald-500"/>تفعيل</>}</button>
                ...
                <tr key={form.$id} className="hover:bg-blue-50/30 group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100">
                      <button onClick={()=>toggleStatus(form)} className="p-2 rounded-lg hover:bg-gray-100 text-gray-400" title={form.status==='active'?'إيقاف':'تفعيل'}>{form.status==='active'?<EyeOff size={16}/>:<Eye size={16}/>}</button>
